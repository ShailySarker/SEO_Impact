
const CareersBanner = () => {
    return (
        <div className="my-12">
            <h2 className="font-bold text-5xl text-center md:mb-36 mb-24">Careers</h2>

            <div className="md:ml-28 ml-6 lg:mr-[900px] md:mr-56 mr-10">
                <h2 className="font-bold md:text-5xl text-3xl pb-7">Join Us</h2>
                <p className="font font-semibold lg:text-2xl md:text-xl text-lg text-slate-500">Lorem ipsum dolor sit amet consectetur. Luctus in convallis odio eu egestas lectus.</p>
                <button className="text-white bg-[#007BFE] md:py-3 md:px-6 p-2 w-36 md:w-40 rounded-xl font-semibold lg:mt-16 mt-12">More about us</button>

            </div>
            <img className="relative lg:bottom-60 md:bottom-28 bottom-12" src="https://i.ibb.co/ss3MmK8/Vector-8.png" alt="" />
        </div>
    );
};

export default CareersBanner;